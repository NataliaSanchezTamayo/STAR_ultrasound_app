//------------------------------------------------------------------------------
// File: PushSourceBitmapSender.cpp
//
...
#define BITMAP_NAME TEXT("image.bmp")
...
/**********************************************
*
*  CPushPinBitmapSender Class
*
*
**********************************************/

CPushPinBitmapSender::CPushPinBitmapSender(HRESULT *phr, CSource *pFilter)
	...
	m_rtFrameLength(FPS_30) // Display 30 bitmap frames per second
{
	...
	TCHAR newDir[MAX_PATH] = TEXT("C:\\Users\\Public");
...
void CPushPinBitmapSender::UpDateHBitmap()
{
...
CPushPinBitmapSender::~CPushPinBitmapSender()
{
...
HRESULT CPushPinBitmapSender::GetMediaType(CMediaType *pMediaType)
{
...
HRESULT CPushPinBitmapSender::DecideBufferSize(IMemAllocator *pAlloc, ALLOCATOR_PROPERTIES *pRequest)
{
	HRESULT hr;
	CAutoLock cAutoLock(m_pFilter->pStateLock());
...
HRESULT CPushPinBitmapSender::FillBuffer(IMediaSample *pSample)
{
	BYTE *pData;
	long cbData;
...
/**********************************************
*
*  CPushSourceBitmapSender Class
*
**********************************************/

CPushSourceBitmapSender::CPushSourceBitmapSender(IUnknown *pUnk, HRESULT *phr)
	: CSource(NAME("PushSourceBitmapSender"), pUnk, CLSID_PushSourceBitmapSender)
{
	// The pin magically adds itself to our pin array.
	m_pPin = new CPushPinBitmapSender(phr, this);
...
CPushSourceBitmapSender::~CPushSourceBitmapSender()
{
	delete m_pPin;
}


CUnknown * WINAPI CPushSourceBitmapSender::CreateInstance(IUnknown *pUnk, HRESULT *phr)
{
	CPushSourceBitmapSender *pNewFilter = new CPushSourceBitmapSender(pUnk, phr);
